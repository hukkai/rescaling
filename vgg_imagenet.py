import torch, argparse, time, torchvision
import models, utils
...
scheduler = utils.warmup_scheduler(base_lr=base_lr, iter_per_epoch=len(train_loader), 
    max_epoch=100 + warmup_epoch, multi_step=args.multi_step, warmup_epoch=warmup_epoch)
...
for epoch in range(100 + warmup_epoch):    
    start = time.time()
    train_loss, train_acc = utils.train(net, train_loader, criterion, optimizer, mean_and_std, scheduler)
